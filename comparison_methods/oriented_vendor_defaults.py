...
def choose_best_orientation_6way(
    mesh_path: str,
    pitch_mm: float = 0.75,
    tie_eps_mm2: float = 1e-6,
    ...
    def _load_trimesh(path: str) -> trimesh.Trimesh:
        ...
        if isinstance(m, trimesh.Scene):
            m = trimesh.util.concatenate(m.dump())
        if not isinstance(m, trimesh.Trimesh):
            raise ValueError(f"Loaded object is not a triangular mesh: {type(m)}")
        return m
    ...
    def _rotate_and_place_on_bed(mesh: trimesh.Trimesh, rx_deg: float, ry_deg: float, rz_deg: float) -> trimesh.Trimesh:
        m = mesh.copy()
    ...
    def _unsupported_down_area_voxel(mesh_on_bed: trimesh.Trimesh, pitch: float) -> Tuple[float, float]:
           
        if pitch <= 0:
            raise ValueError("pitch_mm must be > 0")

        vg = mesh_on_bed.voxelized(pitch)
        try:
            vg = vg.fill()
        except Exception:
            pass
        ...
        below = np.zeros_like(occ, dtype=bool)
        below[:, :, 1:] = occ[:, :, :-1]

        bottom_exposed = occ & (~below)

        ks = np.where(occ)[2]
        k0 = int(ks.min())

        bed_mask = np.zeros_like(bottom_exposed, dtype=bool)
        bed_mask[:, :, k0] = True

        bed_faces = bottom_exposed & bed_mask
        unsupported_faces = bottom_exposed & (~bed_mask)

        area_per_face = float(pitch * pitch)
        unsupported_mm2 = float(unsupported_faces.sum()) * area_per_face
        bed_mm2 = float(bed_faces.sum()) * area_per_face
        return unsupported_mm2, bed_mm2
    ...
    for name, rx, ry, rz in candidates:
        rx, ry, rz = _norm_deg(rx), _norm_deg(ry), _norm_deg(rz)

        m = _rotate_and_place_on_bed(base, rx, ry, rz)
        unsupported_mm2, bed_mm2 = _unsupported_down_area_voxel(m, pitch_mm)
        ...
        rec = {
            ...
            "ry_deg": ry,
            "rz_deg": rz,
            "unsupported_down_mm2": float(unsupported_mm2),
            "bed_contact_mm2": float(bed_mm2),
        }
        results.append(rec)
        ...
        u, ub = rec["unsupported_down_mm2"], best["unsupported_down_mm2"]
        if (u + tie_eps_mm2) < ub:
            best = rec
        elif abs(u - ub) <= tie_eps_mm2:
                                             
            if rec["bed_contact_mm2"] > best["bed_contact_mm2"]:
                best = rec
    ...
    debug = {
        ...
        "tie_eps_mm2": float(tie_eps_mm2),
        "mesh_watertight": bool(base.is_watertight),
        "candidates": results,
        "chosen": best,
        "prusa_semantics": "R = Rz(rz) @ (Ry(ry) @ Rx(rx))",
    }

    return float(best["rx_deg"]), float(best["ry_deg"]), float(best["rz_deg"]), debug
...
def _slice_profile_defaults_with_warning_reruns_and_exports(
    ...
    try:
        ...
        rx_deg, ry_deg, rz_deg, orient_dbg = choose_best_orientation_6way(MODEL_STL, pitch_mm=0.75)
